...
public class CarScanHisActivity extends AppCompatActivity implements XListView.IXListViewListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_car_scan_his);
        initView();
    }
...
//        getData();
    ...
    private void initView() {
        ...
        if (HttpUtil.isNetConn(this)){
            new CarScanHisActivity.LoadCarAsyncTask(this, mListView).execute();
        }else {
            //必须直接添加数据的函数
...
//                new LoadCarAsyncTask(this, mListView).execute();
                if (HttpUtil.isNetConn(CarScanHisActivity.this)){
                    new CarScanHisActivity.LoadCarAsyncTask(CarScanHisActivity.this, mListView).execute();
                }else {
                    //必须直接添加数据的函数
                    mAdapter = new ImageAdapter(CarScanHisActivity.this, getData());
                    mListView.setAdapter(mAdapter);
                }
                onLoad();
            }
    ...
    class LoadCarAsyncTask extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            if (HttpUtil.isNetConn(mContext)) {
                String url = "CarSanHisController/selectByLimt.do?start=" + start + "&limit=" + limit+"&uid="+uid;
                byte[] b = HttpUtil.downloadFromNet(CarScanHisActivity.this, url);  //可变参数params当成一个数组使用，其中的params[0]就是我们传递过来的参数
        ...
        protected void onPostExecute(String result) {
            ...
            if (carInfors.size() != 0) {
                ...
                if (imageAdapter == null) {
                    list = getData1();
                    imageAdapter = new ImageAdapter(CarScanHisActivity.this, list);
