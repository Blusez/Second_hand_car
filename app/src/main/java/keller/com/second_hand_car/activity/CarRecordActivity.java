...
public class CarRecordActivity extends AppCompatActivity implements XListView.IXListViewListener{
...
//        getData();
    ...
    private void initView() {
        ...
        mListView = (XListView) findViewById(R.id.xListView4);
        mListView.setPullLoadEnable(true);

        if (HttpUtil.isNetConn(this)){
            new CarRecordActivity.LoadCarAsyncTask(this, mListView).execute();
        }else {
            //必须直接添加数据的函数
...
//                new LoadCarAsyncTask(this, mListView).execute();
                if (HttpUtil.isNetConn(CarRecordActivity.this)){
                    new CarRecordActivity.LoadCarAsyncTask(CarRecordActivity.this, mListView).execute();
                }else {
                    //必须直接添加数据的函数
                    mAdapter = new ImageAdapter(CarRecordActivity.this, getData());
                    mListView.setAdapter(mAdapter);
                }
                onLoad();
            }
    ...
    class LoadCarAsyncTask extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            if (HttpUtil.isNetConn(mContext)) {
                String url = "SaleOrderController/selectOrder.do?start=" + start + "&limit=" + limit+"&uid="+uid;
                byte[] b = HttpUtil.downloadFromNet(CarRecordActivity.this, url);  //可变参数params当成一个数组使用，其中的params[0]就是我们传递过来的参数
        ...
        protected void onPostExecute(String result) {
            ...
            if (carInfors.size() != 0) {
                ...
                if (imageAdapter == null) {
                    list = getData1();
                    imageAdapter = new ImageAdapter(CarRecordActivity.this, list);
